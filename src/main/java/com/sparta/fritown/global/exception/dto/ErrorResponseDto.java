...
public class ErrorResponseDto extends BaseResponse {
    ...
    public static ErrorResponseDto failure(ErrorCode errorCode) {
        /* 응답 객체를 생성하기 위함.
         * 정적 팩토리 메서드란 생성자를 호출하면서도, 객체 생성 과정을 캡슐화한 메서드
         * 이점: ErrorResponseDto의 내부 구조가 수정 (필드 이름 수정,,) 되어도, 수정할 필요가 없음.
         */
        return new ErrorResponseDto(errorCode.getStatus().value(), errorCode.getCode(), errorCode.getMessage());
    }
}
